...
export const cropsApi = {
  ...
  updateListing: (id: string, data: Partial<CreateListingRequest> & { status?: string; rejectionReason?: string }) =>
    apiClient.put<void>(`/listings/${id}/status`, data),

  /** Returns all Pending_Approval listings — Market Officer queue */
  getPendingListings: () =>
    apiClient.get<CropListing[]>('/listings/pending').then((r) => r.data),
  ...
  /** Orders on a farmer's listings — filtered by farmerId via listing ownership */
  getFarmerOrders: (farmerId: string) =>
    apiClient.get<Order[]>('/orders', { params: { farmerId } }).then((r) => r.data),

  /** Trader cancels their own pending order */
  cancelOrder: (orderId: string) =>
    apiClient.delete<void>(`/orders/${orderId}`),
